...
rooms[1] = Room(name="Office", capacity=5)


# Set up thermal cam logging
...
class pithermalcam:
    ...
    def _add_image_text(self, blob_count=0):
        """Set image text content"""
        if self.use_f:
            temp_min = self._c_to_f(self._temp_min)
            temp_max = self._c_to_f(self._temp_max)
            # Add Blobs to the text string below
            text = f'Tmin={temp_min:+.1f}F Tmax={temp_max:+.1f}F Blobs={blob_count}'
        else:
            # Add Blobs to the text string below
            text = f'Tmin={self._temp_min:+.1f}C Tmax={self._temp_max:+.1f}C Blobs={blob_count}'
    ...
    def display_camera_onscreen(self, sensitivity):
        ...
        self.update_image_frame(sensitivity)
    ...
    def detect_blobs(self, sensitivity):
        """
        Detects hot spots in the image. Sensitivity ranges from 0-255. Higher means it only detects hotter things.
        """
        blob_img = cv2.resize(self._raw_image, (800, 600), interpolation=cv2.INTER_CUBIC)
        blob_img = cv2.flip(blob_img, 1)
        # Apply sensitivity threshold
        _, thresh = cv2.threshold(blob_img, sensitivity, 255, cv2.THRESH_BINARY)
        # Find spot contours
        contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        # Filter out noise
        min_area = 4600
        valid_contours = [c for c in contours if cv2.contourArea(c) > min_area]
        # Draw the rectangle for every contour found
        for contour in valid_contours:
            x, y, w, h = cv2.boundingRect(contour)
            cv2.rectangle(self._image, (x, y), (x + w, y + h), (0, 255, 0), 2)
        return len(valid_contours)

    def update_image_frame(self, sensitivity=140):
        """Pull raw temperature data, process it to an image, and update image text"""
        self._pull_raw_image()
        self._process_raw_image()
        blob_count = self.detect_blobs(sensitivity)
        self._add_image_text(blob_count)
        rooms[1].capacity = blob_count
...
@app.get("/capacity/{room_id}")
def read_capacity(room_id: int):
    thermcam.display_camera_onscreen(sensitivity=140)
    return {
        "room_id": room_id,
